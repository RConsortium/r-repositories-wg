...
function(p, i = TRUE, d = Sys.Date() + 14, recursive = FALSE,
         collapse = FALSE)
{
    d <- max(Sys.Date() + 14, as.Date("2022-01-06"))
    ...
    body <-
        c(info$body,
          "",
          "We have asked for an update fixing the check problems shown at one of",
          sprintf(   "<https://www.stats.ox.ac.uk/pub/bdr/LENGTH1_self/%s.out>", p),
	  sprintf(   "<https://www.stats.ox.ac.uk/pub/bdr/LENGTH1/%s.out>", p),
          if(i)
